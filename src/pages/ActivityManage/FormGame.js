...
export default class FormGame extends React.Component {
  ...
  // 添加报名字段
  addFormMatch = () => {
    let { ticketMatchArr } = this.state;
    ticketMatchArr.push({ name: '', type: '' });
    this.setState({
      ticketMatchArr,
    });
  };
  // 删除报名字段
  removeFormMatch = index => {
    let { ticketMatchArr } = this.state;
    ticketMatchArr.splice(index, 1);
    this.setState({
      ticketMatchArr,
    });
  };
  ...
  handleFormSubmit = e => {
    ...
    this.props.form.validateFields('', (err, values) => {
      if (!err) {
        ...
        for (let i in ticketForm) {
          ticketForm[i].start_time = moment(ticketForm[i].time[0]._d).format('YYYY-MM-DD HH:mm'); //开始时间
          ticketForm[i].end_time = moment(ticketForm[i].time[1]._d).format('YYYY-MM-DD HH:mm'); //结束时间
          if (ticketForm[i].choose) {
      ...
      ticketForm && ticketForm.length > 0
        ? ticketForm.map((item, index) => (
            <div key={index} className={styles.ticketFormItem}>
              ...
              {/*<FormItem {...formItemLayout2} label='轮次时间'>*/}
              {/*{getFieldDecorator(`ticket-time-${index}`, {*/}
              {/*initialValue:*/}
              {/*item.start_time && item.end_time*/}
              {/*? [*/}
              {/*moment(item.start_time, 'YYYY-MM-DD HH:mm'),*/}
              {/*moment(item.end_time, 'YYYY-MM-DD HH:mm'),*/}
              {/*]*/}
              {/*: '',*/}
              {/*rules: [*/}
              ...
              {/*// onChange={e => this.changeTicketForm(e, index, 'time')}*/}
              {/*/>*/}
              {/*)}*/}
              {/*</FormItem>*/}

              {/*<FormItem {...formItemLayout2} label="比赛时间">*/}
              ...
              <h4>报名表单</h4>
              <FormItem {...formItemLayout2} label="报名表单">
                {getFieldDecorator(`ticket-match-${index}`, {
                  initialValue: '',
                })(
                  <Row>
                    <Col>
                ...
                <a onClick={() => this.removeFormMatch(index)}>删除</a>
              </FormItem>

              <a onClick={this.addFormMatch}>添加报名字段</a>
            ...
            <Col xs={24} sm={24} md={24} lg={14}>
              ...
              >
                ...
                <FormItem {...formItemLayout} label={`${title}名称`}>
                  ...
                  })(<Input autoComplete="off" allowClear={true} placeholder="请输入" />)}
                </FormItem>

                {level === '2' ? (
                  <FormItem {...formItemLayout} label={'是否赠票'}>
                    {getFieldDecorator('ticket_id', {
                      rules: [{ required: true, message: '请选择是否赠票' }],
                    })(<Input autoComplete="off" allowClear={true} placeholder="请输入" />)}
                  </FormItem>
                ) : null}
                ...
                <FormItem {...formItemLayout} label={`${title}上架时间`}>
                  {getFieldDecorator('shelf_time', {
                    initialValue: detail.shelf_time
                      ? moment(detail.shelf_time, 'YYYY-MM-DD HH:mm')
                      : '',
                ...
                <FormItem {...formItemLayout} label="列表展示图">
                  {getFieldDecorator('image', {
                    ...
                    rules: [{ required: true, message: '请选择展示图' }],
